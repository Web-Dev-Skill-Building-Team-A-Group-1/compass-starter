...
export function withEntitiesAndSelectMethods<S extends AnyEntity>() {
  return signalStoreFeature(
    ...
    withMethods((store) => ({
      ...
      handleStreamChanges(entitiesToAdd: S[], entitiesToModify: S[], entitiesToRemove: string[]) {
        patchState(store, setEntities(entitiesToAdd, { selectId }));
        patchState(store, setEntities(entitiesToModify, { selectId }));
        patchState(store, removeEntities(entitiesToRemove));
      },
    ...
    filteredEntities = filteredEntities.filter((e) => {
      ...
      const filterValue = !queryParams.find((f) => {
        ...
        switch (rel) {
          ...
          case 'in':
            return val.length > 0 && !val.includes(e[key]);
  ...
  if (entities.length > 0) {
    ...
    await Promise.all(queriesByQueryType.map(async (queriesForOneType) => {
      ...
      await Promise.all(optimizedQueries.filter((q) => !!q).map(async (query) => {
        if (!query.queryParams.some((param) => Array.isArray(param[2]) && param[2].length === 0 && param[1] === 'in')) {
