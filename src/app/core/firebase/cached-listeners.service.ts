import { Inject, Injectable, WritableSignal, inject, DestroyRef, computed } from '@angular/core';
...
import { setEntities, setEntity, removeEntities, SelectEntityId } from '@ngrx/signals/entities';
...
export class CachedListenersService {
   ...
   */
  public async processStreamQuery<T extends AnyEntity>(
    collection: string,
    store,
    ...
    if (connection) {
      ...
      connection = this.createConnection<T>(collection, store, queryParams, queryOptions, destroyRef);
  ...
  private createConnection<T>(collection: string, store, queryParams: QueryParams, queryOptions: QueryOptions, destroyRef: DestroyRef): CachedConnection {
    ...
    this.db.streamEntitiesChanges(collection, queryParams, queryOptions).pipe(
      takeUntil(disconnectObs$),
    ).subscribe((fbPayload: Array<{ type: string, docData: AnyEntity }>) => {
      ...
      store.handleStreamChanges(
        entitiesToAdd,
        [...entitiesToModify, ...entitiesToModify2],
        [...entitiesToRemove, ...entitiesToRemove2],
      );
    });
